...
func rewriteTestCases() []rewriteTestCase {
	return []rewriteTestCase{
		...
		{"docker compose", "docker compose up", "sctx docker compose up", true},
		...
		{"git rebase not in table", "git rebase main", "git rebase main", false},
		...
		{"git cherry-pick not in table", "git cherry-pick abc", "git cherry-pick abc", false},
		...
		{"docker exec not in table", "docker exec c1 sh", "docker exec c1 sh", false},
		...
		{"kubectl exec not in table", "kubectl exec pod -- sh", "kubectl exec pod -- sh", false},
