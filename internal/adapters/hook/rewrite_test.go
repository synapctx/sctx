...
func rewriteTestCases() []rewriteTestCase {
	return []rewriteTestCase{
		...
		{"dig", "dig example.com A", "sctx dig example.com A", true},
		{"psql command", `psql app -c 'select * from jobs'`, `sctx psql app -c 'select * from jobs'`, true},
		{"psql long command", `psql --command='select 1'`, `sctx psql --command='select 1'`, true},
		{"psql list", "psql --list", "sctx psql --list", true},
		{"psql interactive", "psql production", "psql production", false},
		{"psql script unknown finiteness", "psql -f migrations.sql", "psql -f migrations.sql", false},
		{"psql watch", `psql -c '\watch 1'`, `psql -c '\watch 1'`, false},
		{"psql shell meta-command", `psql -c '\! tail -f app.log'`, `psql -c '\! tail -f app.log'`, false},
		{"psql stdin copy", `psql -c 'COPY events FROM STDIN'`, `psql -c 'COPY events FROM STDIN'`, false},
